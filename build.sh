...
for PKG_PATH in $(catkin_topological_order --only-folders); do
  ...
  sbuild --chroot-mode=unshare --no-clean-source --no-run-lintian \
    ...
    --extra-package=/home/runner/apt_repo "$@" || echo "- [$(catkin_topological_order --only-names)](https://raw.githubusercontent.com/$GITHUB_REPOSITORY/$DEB_DISTRO-$ROS_DISTRO/$(basename /home/runner/apt_repo/$(head -n1 debian/changelog | cut -d' ' -f1)*-*T*.build))" >> /home/runner/apt_repo/Failed.md
  )
  COUNT=$((COUNT+1))
  echo "::endgroup::"
done
